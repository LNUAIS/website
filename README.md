...
One dashboard setting, under Workers → website → Settings → Build:

| Setting | Value |
| --- | --- |
| Build command | `npx opennextjs-cloudflare build` |

Deploy command (`npx wrangler deploy`) and production branch (`main`) are
already correct.

The build command matters: Cloudflare defaults to `npm run build`, which is
`next build` and only ever produces `.next/`. The Worker entry point
`.open-next/worker.js` comes from `opennextjs-cloudflare build`, and without it
the deploy step fails with *"The entry-point file at `.open-next/worker.js` was
not found"*.

`npm run build` is deliberately left as plain `next build`, because OpenNext
shells out to `npm run build` internally — pointing that script at the adapter
would recurse forever.

Plus one bucket, once, for pages regenerated by `revalidate`:

```bash
npx wrangler r2 bucket create lnuai-website-cache
```
