...
Cloudflare Workers, via the OpenNext adapter — which keeps ISR and `next/image`
optimisation, both of which a static export would drop.

Pushing to `main` deploys through `.github/workflows/deploy.yml`. It lints and
type-checks first, so a malformed content file fails CI rather than the site.

Two one-time setup steps, both outside this repo:

1. **R2 bucket** for pages regenerated by `revalidate`:
   `npx wrangler r2 bucket create lnuai-website-cache`
2. **Repository secrets** `CLOUDFLARE_API_TOKEN` (Edit Cloudflare Workers
   template) and `CLOUDFLARE_ACCOUNT_ID`, under Settings → Secrets and
   variables → Actions.

Locally:

```bash
npm run preview   # build and serve the worker as Cloudflare will run it
npm run deploy    # build and deploy from your machine
```

The workflow also has a manual trigger, which matters because the events section
splits upcoming from past against the clock — if nothing has been pushed in a
while, a redeploy refreshes it immediately rather than waiting for the hourly
revalidate.
